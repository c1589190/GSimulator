...
public class AgentsApiHandler implements HttpHandler {
    ...
    private void handleRun(HttpExchange exchange) throws IOException {
        ...
        BaseApiHandler.sendJson(exchange, 201,
                ApiResponse.ok("Agent started: " + instance.instanceId(), data));
    }
    ...
    private void handleList(HttpExchange exchange) throws IOException {
        ...
        if (statusStr != null && !statusStr.isBlank()) {
            try { status = AgentStatus.valueOf(statusStr.toUpperCase()); }
    ...
    private void handleEvents(HttpExchange exchange, String instanceId) throws IOException {
        ...
        sseManager.streamEvents(exchange, instanceId,
                agent.sessionId(), agent.taskId(), agentsManager);
    }
    ...
    private void handleOutput(HttpExchange exchange, String instanceId) throws IOException {
        ...
        if (output == null) {
            if (agent.status() == AgentStatus.RUNNING || agent.status() == AgentStatus.PENDING) {
                BaseApiHandler.sendError(exchange, 202,
                        "Agent still running. Check events at /api/agents/" + instanceId + "/events");
                return;
            }
    ...
    private void handleCancel(HttpExchange exchange, String instanceId) throws IOException {
        ...
        if (cancelled) {
            ...
            BaseApiHandler.sendError(exchange, 400,
                    "Cannot cancel agent in status: " + agent.status());
        }
    }
