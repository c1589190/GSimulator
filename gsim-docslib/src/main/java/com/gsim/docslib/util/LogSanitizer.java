...
public final class LogSanitizer {
    ...
    /** 常用的 Bearer token 模式 */
    private static final java.util.regex.Pattern AUTH_BEARER =
            java.util.regex.Pattern.compile("(?i)(Authorization\\s*:\\s*Bearer\\s+)\\S+");

    /** API Key 常见 key=value / key: value 模式 */
    private static final java.util.regex.Pattern API_KEY_PAIR =
            java.util.regex.Pattern.compile("(?i)(api[_-]?key\\s*[:=]\\s*['\"\\s]?)\\S+");

    /** JSON "apiKey": "..." 模式 */
    private static final java.util.regex.Pattern API_KEY_JSON =
            java.util.regex.Pattern.compile("(\"apiKey\"\\s*:\\s*\")[^\"]+");

    /** HTTP Header Bearer <token> 模式（无 "Authorization:" 前缀，仅 token 本身） */
    private static final java.util.regex.Pattern BEARER_TOKEN = java.util.regex.Pattern.compile("(?i)(Bearer\\s+)\\S+");
