...
class NoSlf4jSimpleDependencyTest {
    ...
    void noSlf4jSimpleInPom() throws Exception {
        ...
        assertTrue(Files.exists(pomPath), "pom.xml should exist at project root (working directory)");

        String content = Files.readString(pomPath);

        // slf4j-simple (both 1.x and 2.x) would conflict with log4j-slf4j2-impl
        assertFalse(
                content.contains("slf4j-simple"),
    ...
    void onlyLog4jSlf4j2ImplAsBinding() throws Exception {
        ...
        assertTrue(
                content.contains("log4j-slf4j2-impl"),
                "pom.xml must contain log4j-slf4j2-impl as the SLF4J 2.x binding");

        // No other SLF4J bindings
        assertFalse(
                content.contains("log4j-slf4j-impl"),
                "Must use log4j-slf4j2-impl (SLF4J 2.x compatible), " + "NOT log4j-slf4j-impl (SLF4J 1.x)");
        assertFalse(content.contains("logback-classic"), "logback-classic must be removed");
    }
    ...
    void noOtherSlf4jBindings() throws Exception {
        ...
        assertFalse(content.contains("slf4j-nop"), "Must not contain slf4j-nop (no-op binding)");
        assertFalse(content.contains("slf4j-jdk14"), "Must not contain slf4j-jdk14 (java.util.logging binding)");
        assertFalse(content.contains("slf4j-jcl"), "Must not contain slf4j-jcl (commons-logging binding)");
    }
}
