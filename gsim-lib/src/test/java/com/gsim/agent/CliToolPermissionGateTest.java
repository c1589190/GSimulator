...
class CliToolPermissionGateTest {
    ...
    void inputYReturnsAllowOnce() {
        ...
        var req = new ToolConfirmationRequest(
                "write_element", ToolCategory.MUTATING, "写入工具：write_element 将修改数据，需要用户确认。", Map.of(), null);
    ...
    void inputYesReturnsAllowOnce() {
        ...
        var gate = new CliToolPermissionGate(captureOutput(), in);
        var req =
                new ToolConfirmationRequest("simulation_content_append", ToolCategory.MUTATING, "写入工具", Map.of(), null);
        assertEquals(ConfirmationChoice.ALLOW_ONCE, gate.askConfirmation(req));
    }
    ...
    void emptyInputReturnsAllowOnce() {
        ...
        var gate = new CliToolPermissionGate(captureOutput(), in);
        var req = new ToolConfirmationRequest("branch_create_child", ToolCategory.MUTATING, "写入工具", Map.of(), null);
        assertEquals(ConfirmationChoice.ALLOW_ONCE, gate.askConfirmation(req));
    }
    ...
    void inputAReturnsAllowAll() {
        ...
        var gate = new CliToolPermissionGate(captureOutput(), in);
        var req = new ToolConfirmationRequest("write_element", ToolCategory.MUTATING, "写入工具", Map.of(), null);
    ...
    void inputNReturnsDeny() {
        ...
        var gate = new CliToolPermissionGate(captureOutput(), in);
        var req = new ToolConfirmationRequest("knowledge_delete", ToolCategory.DESTRUCTIVE, "破坏性操作", Map.of(), null);
