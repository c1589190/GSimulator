...
class ToolLoopNoToolReminderMentionsFinishActionTest {
    ...
    void reminderMentionsFinishAction() {
        ...
        assertTrue(reminder.contains("finish_action"), "Reminder must mention finish_action: " + reminder);
        assertTrue(reminder.contains("不要直接用普通自然语言结束"), "Reminder must warn against plain NL ending");
    }
    ...
    void reminderMentionsBusinessTools() {
        ...
        assertTrue(
                reminder.contains("业务工具") || reminder.contains("工具"),
    ...
    void reminderIsNotEmpty() {
        ...
        assertFalse(reminder.contains("player_action_list"), "null user input should not trigger player_action hint");
    }
    ...
    void reminderRequiresFullAnswerInFinishActionMessage() {
        ...
        assertTrue(
                reminder.contains("finish_action") || reminder.contains("完整"),
                "Reminder must mention finish_action or 完整: " + reminder);
        assertTrue(
                reminder.contains("把完整最终回复放入 message") || reminder.contains("message"),
    ...
    void reminderForbidsReferringToInvisibleContent() {
        ...
        assertTrue(reminder.contains("禁止使用"), "Reminder must forbid referencing invisible content");
        assertTrue(
                reminder.contains("以上") || reminder.contains("如上") || reminder.contains("刚才"),
