...
class ToolLoopDetectsPlainAnswerWithoutFinishActionTest {
    ...
    void plainAnswerDetected() {
        var result = ToolLoopDebug.detectFinishIntent(0, 0, false, "当前系统状态正常，你可以继续操作。", false);
    ...
    void apiToolCallReturnsNone() {
        var result = ToolLoopDebug.detectFinishIntent(1, 0, false, "调用工具中...", false);
    ...
    void textFallbackToolReturnsNone() {
        var result = ToolLoopDebug.detectFinishIntent(0, 1, false, "{\"tool\":\"echo\"}", false);
        assertEquals(ToolLoopDebug.FinishIntent.NONE, result);
    }
    ...
    void invalidBracketIntentDetected() {
        var result = ToolLoopDebug.detectFinishIntent(0, 0, true, "[调用 player_action_list] {...}", false);
    ...
    void finishRejectedTakesPriority() {
        ...
        var result = ToolLoopDebug.detectFinishIntent(1, 0, false, "完成后显示...", true);
    ...
    void emptyDraftReturnsNone() {
        var result = ToolLoopDebug.detectFinishIntent(0, 0, false, "", false);
        assertEquals(ToolLoopDebug.FinishIntent.NONE, result);
    }
    ...
    void nullDraftReturnsNone() {
        var result = ToolLoopDebug.detectFinishIntent(0, 0, false, null, false);
        assertEquals(ToolLoopDebug.FinishIntent.NONE, result);
    }
}
