...
class CliProgressShowsSelectedToolTest {
    ...
    void toolSelectedIsVisible() {
        ...
        assertTrue(line.contains("player_action_list"), "Should include tool name: " + line);
        assertTrue(line.contains("🔧"), "Should have wrench emoji: " + line);
    }
    ...
    void toolExecutingIsVisible() {
        var event = new AgentProgressEvent(
                ...
                1,
                5,
                "正在执行工具：player_action_list",
                Map.of("tool", "player_action_list"));
        String line = CliAgentProgressSink.format(event);
        assertNotNull(line, "TOOL_EXECUTING should be visible");
        assertTrue(line.contains("player_action_list"), "Should include tool name: " + line);
        assertTrue(line.contains("执行中"), "Should show executing status: " + line);
    }
    ...
    void toolFailedIsVisible() {
        ...
        assertTrue(line.contains("❌"), "Should have cross emoji: " + line);
    }
