...
class CliStreamPreviewRendererTest {
    ...
    void sinkPrintsErrorOnStreamFailedWithoutRenderer() {
        ...
        assertTrue(errOutput.contains("连接超时"), "应包含错误详情，实际: " + errOutput);
    }
    ...
    void formatQuietModePreservesErrorsAndMessages() {
        ...
        assertNull(CliAgentProgressSink.format(AgentProgressEvent.contextLoaded(1, 32, 5000, 3)));
        assertNull(CliAgentProgressSink.format(AgentProgressEvent.waitingLlm(1, 32)));

        // Visible: tool selected / success (now shown to user)
        assertNotNull(CliAgentProgressSink.format(AgentProgressEvent.toolSelected(1, 32, "query_keyword")));
        assertNotNull(CliAgentProgressSink.format(AgentProgressEvent.toolSuccess(1, 32, "query_keyword")));

        // Visible: public message
        String pubMsg = CliAgentProgressSink.format(AgentProgressEvent.publicMessage("Hello world"));
        assertEquals("Hello world", pubMsg);

        // Visible: tool failed
        assertNotNull(CliAgentProgressSink.format(AgentProgressEvent.toolFailed(1, 32, "bad", "err")));

        // Quiet: finish accepted
        assertNull(CliAgentProgressSink.format(AgentProgressEvent.finishAccepted(1, 32)));
    }
    ...
    void fullInlineSequenceFromStartToComplete() {
        ...
        assertTrue(
                afterDeltas.contains("第一段") || afterDeltas.contains("第二段") || afterDeltas.contains("第三段"),
