...
class WebImportManagerTest {
    ...
    void testExecute_InvalidUrl() throws Exception {
        ...
        WebImportRequest request =
                WebImportRequest.builder(new URI("not-a-valid-url")).build();
    ...
    void testRequestBuilder_Defaults() throws Exception {
        WebImportRequest request =
                WebImportRequest.builder(new URI("https://example.com")).build();
    ...
    void testResultSummary() {
        WebImportResult result = new WebImportResult(
                "https://example.com", "example.com", 5, 0, 2, 5, List.of(), List.of("error1"), false, "generic");
