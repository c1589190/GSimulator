...
class SessionPoolTest {
    ...
    void pushNodeCreatesAndQueries() {
        SessionNode node = pool.pushNode("s1", NodeType.USER_INPUT, Map.of("text", "hello"));
    ...
    void pushNodeWithParentId() {
        SessionNode parent = pool.pushNode("s1", NodeType.USER_INPUT, Map.of("text", "查询"));
        SessionNode child = pool.pushNode("s1", parent.nodeId(), NodeType.TOOL_CALL, Map.of("tool", "query"));
    ...
    void updateNodeUpdatesPayload() {
        SessionNode node = pool.pushNode("s1", NodeType.LLM_STREAMING, new ConcurrentHashMap<>());
    ...
    void transitionStatusChangesStatus() {
        SessionNode node = pool.pushNode("s1", NodeType.TOOL_CALL, new ConcurrentHashMap<>());
