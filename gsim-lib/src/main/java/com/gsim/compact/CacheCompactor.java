...
public class CacheCompactor {
    ...
    public String compact(CacheSession session, AgentProgressSink progressSink) {
        ...
        progressSink.onProgress(
                AgentProgressEvent.publicMessage("  📦 压缩 " + messages.size() + " 条消息 → " + chunks.size() + " 段"));

        // 3. 逐段压缩（流式）
        List<String> summaries = new ArrayList<>();
        for (int i = 0; i < chunks.size(); i++) {
            progressSink.onProgress(AgentProgressEvent.publicMessage("  🔄 压缩第 " + (i + 1) + "/" + chunks.size() + " 段（"
                    + chunks.get(i).length() + " 字符）…"));
    ...
    private String compactChunk(String chunk, int index, int total, AgentProgressSink sink) {
        String userPrompt =
                total == 1 ? "请总结以下对话历史：\n\n" + chunk : "请总结以下对话历史的第 " + (index + 1) + "/" + total + " 部分：\n\n" + chunk;

        List<LlmMessage> messages = List.of(LlmMessage.system(SUMMARIZE_SYSTEM), LlmMessage.user(userPrompt));
        ...
        try {
            while (!pool.isComplete()) {
                ...
                if (!r.isEmpty()) {
                    String lastR = "";
                    if (!r.equals(lastR)) {
                        sink.onProgress(AgentProgressEvent.llmReasoningDelta(
                                "compact-" + compactorId, r.substring(lastR.length())));
                    }
                }
                Thread.sleep(50);
            }
    ...
    private String mergeSummaries(List<String> summaries, AgentProgressSink sink) {
        sink.onProgress(AgentProgressEvent.publicMessage("  🔄 合并 " + summaries.size() + " 段摘要…"));
