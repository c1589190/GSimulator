 ...
 * @param data      事件数据
 */
public record GSimEvent(String sessionId, String taskId, String type, Instant time, Map<String, Object> data) {
    ...
    public GSimEvent {
        ...
        // defensive copy
        data = data != null ? Collections.unmodifiableMap(new LinkedHashMap<>(data)) : Collections.emptyMap();
    }

    // ---- 工厂方法 ----

    /**
     * 创建仅含 sessionId 和 type 的简易事件。
     *
     * @param sessionId 会话 ID
     * @param type      事件类型
     * @return 新构建的事件实例
     ...
     *
     * @param key 数据字段键名
     * @return 字段的字符串值，不存在时返回 {@code null}
     */
