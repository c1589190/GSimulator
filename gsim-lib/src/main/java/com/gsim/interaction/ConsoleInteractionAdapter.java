...
public class ConsoleInteractionAdapter {
     ...
     * @param manager 交互管理器
     * @param session 当前交互会话
     * @param dataDir 数据目录路径（用于 JLine 历史文件存储），可为 {@code null}
     */
    ...
    private void initJline() {
        ...
        try {
            ...
            try {
                ...
                log.warn(
                        "JLine unavailable (normal: {}, dumb: {}), falling back to BufferedReader",
                        e1.getMessage(),
                        e2.getMessage());
            }
        }
    ...
    /**
     * 启动 CLI REPL（Read-Eval-Print Loop）主循环。
     * <p>持续读取用户输入，清洗后路由到对应命令处理器或默认走 /chat 发送消息。
     * 支持 /exit 或 /quit 退出循环，Ctrl+C 也触发退出。</p>
     */
    public void start() {
        printWelcome();
    ...
    private void handleCommand(String cmdName, String rawInput) {
        ...
        switch (cmdName) {
            ...
            default -> displayResult(
                    InteractionResult.fail("Unknown command: /" + cmdName + ". Type /help for available commands."));
        }
    }
    ...
    private String extractCommandName(String rawInput) {
        ...
        return rawInput.trim().substring(1).split("\\s+", 2)[0].toLowerCase(Locale.ROOT);
    }
}
