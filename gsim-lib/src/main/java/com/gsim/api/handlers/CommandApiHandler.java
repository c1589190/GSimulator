...
public class CommandApiHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            eventBus.publish(GSimEvent.of(sessionId, "command_started", Map.of("command", request.command())));
            ...
            if (result.success()) {
                ...
                eventBus.publish(GSimEvent.of(sessionId, "command_error", Map.of("error", result.message())));
                BaseApiHandler.sendJson(exchange, 400, ApiResponse.fail(result.message(), data));
            }
        } catch (Exception e) {
            eventBus.publish(GSimEvent.of(sessionId, "command_error", Map.of("error", e.getMessage())));
