        ...
        try {
            ...
            if (segs.length == 0) {
                if ("GET".equals(method)) {
                    ...
                    InteractionResult result =
                            ctx.getInteractionManager().handle("/exp add " + title + " " + content, session);
                    BaseApiHandler.sendOk(
                            exchange,
                            result.success() ? "Experience added" : "Add failed",
                ...
                BaseApiHandler.sendOk(
                        exchange,
                        result.success() ? "Experience found" : "Experience not found",
