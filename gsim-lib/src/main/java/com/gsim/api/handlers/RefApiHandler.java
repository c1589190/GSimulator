...
public class RefApiHandler implements HttpHandler {
    ...
    public RefApiHandler(
            Path worldsDir, Supplier<String> activeWorldId, Path importDir, Supplier<com.gsim.doc.DocStore> docStore) {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if (ref == null || ref.isBlank()) {
            BaseApiHandler.sendError(
                    exchange, 400, "Query parameter 'ref' is required. Format: @import:|@world:|@doc:");
            return;
        }
        ...
        try {
            ResolvedRef resolved =
                    RefResolver.resolve(ref, worldsDir, resolvedWorldId, importDir, docStore.get(), cacheDir);
            ...
            if (BaseApiHandler.isTextFormat(exchange)) {
                BaseApiHandler.sendTextOk(exchange, "# " + resolved.title() + "\n\n" + resolved.content());
                return;
            }
