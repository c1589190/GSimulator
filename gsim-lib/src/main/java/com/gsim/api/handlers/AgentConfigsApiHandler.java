...
public class AgentConfigsApiHandler implements HttpHandler {
    ...
    private void handleCreate(HttpExchange exchange) throws IOException {
        ...
        if (result.success()) {
            BaseApiHandler.sendJson(
                    exchange, 201, ApiResponse.ok(result.message(), Map.of("configId", result.message())));
        } else {
    ...
    private void handleDelete(HttpExchange exchange, String configId) throws IOException {
        ...
        if (result.success()) {
            BaseApiHandler.sendOk(exchange, result.message(), Map.of("configId", configId, "action", "deleted"));
        } else {
            BaseApiHandler.sendError(exchange, 400, result.message());
        }
    }
