...
public class CliToolPermissionGate implements ToolPermissionGate {
    ...
    /**
     * 使用 System.out 和 System.in 创建 CLI 工具权限门禁。
     */
    ...
    public ConfirmationChoice askConfirmation(ToolConfirmationRequest request) {
        ...
        if (request.params() != null && !request.params().isEmpty()) {
            out.println("╠══════════════════════════════════════╣");
            out.println("║  参数:                               ║");
            request.params().forEach((k, v) -> out.printf("║    %s=%s%n", truncate(k, 10), truncate(v, 20)));
        }
        out.println("╠══════════════════════════════════════╣");
        out.println("║  [Y] 允许一次                        ║");
        ...
        try {
            ...
            String choice = line.trim().toLowerCase(Locale.ROOT);
