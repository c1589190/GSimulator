...
public class CliStreamPreviewRenderer {
     ...
     * @param maxChars       内容最大截断字符数
     * @param showReasoning  是否显示 reasoning 内容
     */
    public CliStreamPreviewRenderer(PrintStream out, boolean enabled, int maxChars, boolean showReasoning) {
        this.out = out;
    ...
    /**
     * 工厂方法：从 AppConfig 创建渲染器实例。
     *
     * @param config AppConfig 配置对象
     * @param out    输出流
     * @return CliStreamPreviewRenderer 实例
     */
    public static CliStreamPreviewRenderer fromConfig(com.gsim.app.AppConfig config, PrintStream out) {
     ...
     *
     * @param streamId 流 ID，用于区分多路流
     * @param text     要追加的 content 文本
     */
