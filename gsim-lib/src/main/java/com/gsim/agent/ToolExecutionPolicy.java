...
public class ToolExecutionPolicy {
    ...
    public ToolExecutionDecision validateBeforeExecute(
        ...
        if (category == ToolCategory.READ_ONLY) {
            return ToolExecutionDecision.allow("只读工具，允许直接执行。", category, true);
        }

        // Rule 3: DESTRUCTIVE → 永远需要确认
        if (category == ToolCategory.DESTRUCTIVE) {
            return ToolExecutionDecision.needConfirmation("⚠ 破坏性操作：" + toolName + " 可能删除/覆盖数据，需要用户确认。", category, true);
        }

        // Rule 4: MUTATING → 需确认（除非 allowAllMutations）
        if (category == ToolCategory.MUTATING) {
            if (allowAllMutations) {
                return ToolExecutionDecision.allow("写入工具，本轮已授权全部写入。", category, true);
            }
            return ToolExecutionDecision.needConfirmation("写入工具：" + toolName + " 将修改数据，需要用户确认。", category, true);
        }
    ...
    /**
     * 构建被拒工具的回灌消息，引导 LLM 重写请求。
     *
     * @param toolName 被拒绝的工具名称
     * @param decision 执行决策（含拒绝原因）
     * @param route    当前路由决策（含允许的工具列表）
     * @return 回灌给 LLM 的提示文本
     */
