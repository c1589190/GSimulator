...
public final class ToolLoopDebug {
    ...
    static void logLlmResult(
            Logger log, String loopName, int round, String content, int apiToolCallCount, String finishReason) {
        ...
        sb.append("finishReason=")
                .append(finishReason != null ? finishReason : "n/a")
                .append('\n');
    ...
    static void logToolExtraction(
            Logger log,
            String loopName,
            int round,
            List<ParsedToolCall> tools,
            int apiToolCallCount,
            int textFallbackToolCallCount,
        ...
        sb.append("round=").append(round).append('\n');
        sb.append("apiToolCallCount=").append(apiToolCallCount).append('\n');
        sb.append("textFallbackToolCallCount=")
                .append(textFallbackToolCallCount)
                .append('\n');
    ...
    static void logToolCall(
            Logger log, String loopName, int round, String toolName, String argsPreview, String source) {
        if (!log.isDebugEnabled()) return;
        StringBuilder sb = new StringBuilder();
        sb.append("\n=== TOOL_LOOP TOOL_CALL ===\n");
        sb.append("loop=").append(loopName).append('\n');
    ...
    static void logFinishAction(
            Logger log, String loopName, int round, String status, String messagePreview, int messageChars) {
        if (!log.isDebugEnabled()) return;
        StringBuilder sb = new StringBuilder();
    ...
    static void logNoToolAbort(Logger log, String loopName, int round, String reason, int consecutiveNoToolRounds) {
        if (!log.isDebugEnabled()) return;
        StringBuilder sb = new StringBuilder();
    ...
    static void logInvalidToolIntent(Logger log, String loopName, int round, int consecutiveInvalid, String content) {
        if (!log.isDebugEnabled()) return;
        StringBuilder sb = new StringBuilder();
    ...
    static void logFinishActionWithOtherToolsRejected(
            Logger log, String loopName, int round, java.util.List<ParsedToolCall> allParsed) {
        if (!log.isDebugEnabled()) return;
        StringBuilder sb = new StringBuilder();
        sb.append("\n=== TOOL_LOOP FINISH_ACTION ===\n");
        sb.append("loop=").append(loopName).append('\n');
        ...
        sb.append("allTools=");
        sb.append(allParsed.stream().map(ParsedToolCall::tool).collect(java.util.stream.Collectors.joining(", ")));
        sb.append("\n=== TOOL_LOOP FINISH_ACTION END ===");
        log.debug(sb.toString());
    }
    ...
    static void logFinalText(Logger log, String loopName, String source, String finalText) {
        if (!log.isDebugEnabled()) return;
    ...
    static void logContextLoad(
        ...
        sb.append("fullWorldContextLoaded=")
                .append(contextMeta.fullWorldContextLoaded())
                .append('\n');
        ...
        sb.append("\n---\n");
        sb.append("messageCounts: system=")
                .append(systemCount)
                .append(" user=")
                .append(userCount)
                .append(" assistant=")
                .append(assistantCount)
                .append('\n');
        sb.append("messageChars=").append(messageChars).append('\n');
        sb.append("toolsJsonChars=").append(toolsJsonChars).append('\n');
        sb.append("estimatedRequestChars=")
                .append(messageChars + toolsJsonChars)
                .append('\n');
        sb.append("\nbranchPath=").append(contextMeta.branchPath()).append('\n');
        sb.append("currentBranchLoaded=")
                .append(contextMeta.currentBranchLoaded())
                .append('\n');
        sb.append("parentBranchesLoaded=")
                .append(contextMeta.loadedParentBranches())
                .append('\n');
        sb.append("tools=")
                .append(toolDefs.stream().map(com.gsim.llm.ToolDef::name).collect(java.util.stream.Collectors.toList()))
                .append('\n');
    ...
    static void logTaskBrief(
        ...
        sb.append("lastToolName=")
                .append(lastToolName != null ? lastToolName : "none")
                .append('\n');
    ...
    private static ExpectedNextStep computeExpectedNextStep(boolean hasToolResult, boolean lastToolSuccess) {
        if (!hasToolResult) {
    ...
    static String inferUserIntent(String userText) {
        ...
        String s = userText;
        if (s.contains("玩家行动") || s.contains("行动记录") || s.contains("有没有行动") || s.contains("player action")) {
     ...
     * 不做复杂语义判断，纯信号检测。
     */
    static FinishIntent detectFinishIntent(
            int apiToolCallCount,
            int textFallbackCount,
            boolean invalidToolIntent,
            String cleanedDraft,
        ...
        if (apiToolCallCount == 0 && textFallbackCount == 0 && cleanedDraft != null && !cleanedDraft.isBlank()) {
    ...
    static String buildNoToolReminder(String userInput) {
        ...
        if (isPlayerActionQuery(userInput)) {
            base += "\n\n用户正在询问玩家行动记录。" + "请调用 finish_action 返回结果。";
        }
    ...
    static boolean isInvalidToolIntent(String content) {
        ...
        if (content.contains("{branchId=") || content.contains("{mode=") || content.contains("{status=")) return true;
        // 包含 "调用 xxx 工具" 等口头工具意图
        if (content.contains("调用 ")
                && (content.contains("工具")
                        || content.contains(" player_action")
                        || content.contains(" knowledge_")
                        || content.contains(" finish_action")
    ...
    static String invalidToolIntentAbortError(int consecutive) {
        return "[错误] Agent 连续 " + consecutive + " 次输出非法工具调用文本，未能生成可执行 tool_calls。" + "请检查 ToolLoop prompt 或模型输出。";
    }

    // ===== 工具路由 / 执行策略 / 确认决策 =====

    /** 记录本轮工具路由决策。 */
    static void logToolRouteDecision(
            Logger log,
            String loopName,
            int round,
            UserIntent userIntent,
    ...
    /** 记录单个工具的执行策略校验。 */
    static void logToolExecutionPolicy(
            Logger log, String loopName, int round, String toolName, ToolExecutionDecision decision) {
        if (!log.isDebugEnabled()) return;
        StringBuilder sb = new StringBuilder();
    ...
    /** 记录用户对工具确认请求的选择。 */
    static void logToolPermissionDecision(
            Logger log, String loopName, int round, String toolName, ConfirmationChoice choice) {
        if (!log.isDebugEnabled()) return;
        StringBuilder sb = new StringBuilder();
