...
public class FinishActionTool implements AgentTool {
    ...
    /**
     * 有效的 {@code status} 枚举值列表。
     * <ul>
     *   <li>{@code success} — 全部完成</li>
     *   <li>{@code partial} — 部分完成</li>
     *   <li>{@code failed} — 执行失败</li>
     *   <li>{@code needs_user_input} — 需要用户补充信息</li>
     * </ul>
     */
    public static final List<String> VALID_STATUSES = List.of("success", "partial", "failed", "needs_user_input");
    ...
    public java.util.Map<String, Object> getParameters() {
        ...
        props.put(
                ...
                java.util.Map.of(
                        ...
                        "string",
                        "enum",
                        java.util.List.of("success", "partial", "failed", "needs_user_input"),
                        "description",
                        "本轮操作状态：success（全部完成）、partial（部分完成）、failed（执行失败）、" + "needs_user_input（需要用户补充信息/做出选择）。"));
        props.put(
                "message",
                java.util.Map.of(
                        "type", "string", "description", "最终展示给用户的完整回复正文。必须自包含，" + "不得使用「以上」「如上」「刚才」「前文」等引用不可见内容。"));
        props.put(
                "summary",
                java.util.Map.of(
                        "type", "string", "description", "本轮操作的简短摘要（可选，但强烈建议提供）。" + "一句话总结你做了什么，用蓝色高亮显示在终端上供用户快速浏览。"));
        return com.gsim.llm.ToolDef.strictSchema(props, java.util.List.of("status", "message"));
    }
    ...
    public ToolResult execute(ToolCall call) {
        ...
        if (status.isBlank()) {
            return ToolResult.fail(NAME, "status 不能为空。必须是 success / partial / failed / needs_user_input 之一。");
        }
        if (!VALID_STATUSES.contains(status)) {
            return ToolResult.fail(
                    NAME, "无效的 status '" + status + "'。必须是 success / partial / failed / needs_user_input 之一。");
        }
        if (message.isBlank()) {
        ...
        items.add(new ToolResult.Item("finish_action: " + status, NAME, message, 1.0));
        if (!summary.isBlank()) {
