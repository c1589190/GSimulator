...
/**
 * ListLlmProvidersTool -- 列出 llms.json 中所有可用的 LLM Provider。
 *
 * <p>供 Orchestrator 在创建 SubAgent 前了解可选的 LLM 供应，包括 provider ID、
 * 名称、模型、Base URL、温度、是否支持原生推理等信息。
 */
public class ListLlmProvidersTool implements AgentTool {
    ...
    public String description() {
        return "列出 llms.json 中所有可用的 LLM Provider（ID、名称、模型、Base URL 等）。" + "在创建 SubAgent 配置前先调用此工具了解可选 provider。";
    }

    @Override
    public Map<String, Object> getParameters() {
        return com.gsim.llm.ToolDef.strictSchema(Map.of(), List.of());
    }
        ...
        try {
            ...
            if (providers.isEmpty()) {
                return ToolResult.ok(
                        NAME, List.of(new ToolResult.Item("info", NAME, "暂无可用 LLM Provider。请检查 llms.json。", 1.0)));
            }
            ...
            return ToolResult.ok(NAME, List.of(new ToolResult.Item("providers", NAME, sb.toString(), 1.0)));
        } catch (Exception e) {
