...
public class OrchestratorAgent extends AbstractAgent {
    ...
    static String stripRawToolJson(String text) {
        ...
        // 移除内联 fenced blocks
        result = result.replaceAll("```[a-z]*\\s*\\{\"tool\"[^`]*```\\s*", "[工具调用已执行]");

        // 移除裸露的 {"tool":"..."} JSON（不在 fence 中但仍是 raw JSON）
        result = result.replaceAll(
                "\\{\"tool\"\\s*:\\s*\"[^\"]+\"\\s*,\\s*\"args\"\\s*:\\s*\\{[^}]*\\}\\}", "[工具调用已执行]");
     ...
     */
    static String guardSuccessClaimWithoutToolBacking(String finalText, List<ToolCallRecord> toolCalls, int toolRound) {
        ...
        // 没有工具执行却有成功宣称 → 追加警告
        return finalText + "\n\n⚠️ [系统提示] 以上回复包含操作成功宣称，" + "但在本轮对话中未检测到对应的工具执行记录。" + "如确有操作需要执行，请重新输入指令。";
    }
     ...
     *
     * @param message finish_action 的 message 字段内容
     * @return null 表示验证通过，非 null 表示错误描述信息
     */
    protected static String validateFinishActionMessage(String message) {
        ...
        if (message.contains("[工具调用已执行]")) {
            return "finish_action.message 包含 [工具调用已执行] 占位符，" + "请用自然语言描述操作结果，重新调用 finish_action。";
        }
        ...
        if (message.contains("```json") || (message.contains("```") && message.contains("\"tool\""))) {
            return "finish_action.message 包含 fenced JSON tool call，" + "请移除所有 ``` 代码块，用自然语言重写，重新调用 finish_action。";
        }
        if (message.matches("(?s).*\\{\"tool\"\\s*:\\s*\"[^\"]+\".*\\}.*")) {
            return "finish_action.message 包含裸 JSON tool call，" + "请移除所有 {\"tool\":...} 内容，用自然语言重写，重新调用 finish_action。";
        }
        // 检测裸 JSON（非 tool call，含 { 但不含 "tool": → 可能是 raw tool output 泄露）
        String trimmed = message.strip();
        if (trimmed.startsWith("{") && !trimmed.contains("\"tool\"")) {
            return "finish_action.message 包含裸 JSON 工具输出（不含 tool 字段），" + "请用自然语言重写，重新调用 finish_action。";
        }
        // 检测伪造的 {key=value} 模式（model 幻觉生成的结果）
        if (message.matches(
                "(?s).*\\{(branchId|mode|status|title|createdBranchId|activeBranchId)\\s*=\\s*[^}]+\\}.*")) {
    ...
    private static List<ParsedToolCall> fromApiToolCalls(List<LlmToolCall> toolCalls) {
        ...
        for (LlmToolCall tc : toolCalls) {
            result.add(new ParsedToolCall(tc.name(), tc.arguments() != null ? tc.arguments() : Map.of()));
        }
        return result;
    }
    ...
    static String stripFakeBracketToolResult(String text) {
        ...
        // 移除 [工具结果] ... [工具结果] 块（含内容）
        String result = text.replaceAll("(?s)\\[工具结果\\][^\\[]*?(?=\\[工具结果\\]|\\n\\n|$)", "");
        // 移除孤立的 [工具结果] 标记
     ...
     * @param type    消息子类型（tool_call / tool_result / chat_response 等）
     * @param content 消息内容
     */
    public record MessageTrace(String role, String type, String content) {}

    /**
     * 推演结果 — 基于已渲染的上下文执行推演后返回。
     *
    ...
    /**
     * 对话模式结果 — 不覆盖任何章节，只返回 LLM 回复。
     *
     * @param success      是否成功完成
     * @param finalText    最终输出文本
     * @param toolCalls    工具调用记录列表
     * @param trace        消息追踪列表
     * @param errorMessage 错误消息（失败时非 null）
     */
    ...
    private ChatResult runToolLoop(
        ...
        while (toolRound <= maxToolRounds) {
            ...
            log.debug(
                    "[TOOL_LOOP_TRACE] engine=runToolLoop round={} requestTools={} activeGroups={}",
                    toolRound,
                    roundToolDefs.size(),
                    groupManager.activeGroupKeys());

            // progress: 上下文加载完毕
            int totalMessageChars = 0;
            for (var m : messages) {
                if (m.content() != null) totalMessageChars += m.content().length();
            }
            int toolsJsonChars = ToolLoopDebug.estimateToolsJsonChars(roundToolDefs);
            progressSink.onProgress(AgentProgressEvent.contextLoaded(
                    toolRound, maxToolRounds, totalMessageChars + toolsJsonChars, roundToolDefs.size()));

            // progress: 等待 LLM
            progressSink.onProgress(AgentProgressEvent.waitingLlm(toolRound, maxToolRounds));

            LlmRequest request =
                    new LlmRequest(null, new ArrayList<>(messages), 0.3, 2048, roundToolDefs, roundToolChoice);
            ...
            int apiToolCallCount =
                    response.hasApiToolCalls() ? response.toolCalls().size() : 0;
            ...
            ToolLoopDebug.logLlmResult(log, "runToolLoop", toolRound, content, apiToolCallCount, finishReason);
            ...
            ToolLoopDebug.logToolExtraction(
                    log, "runToolLoop", toolRound, allParsed, apiToolCallCount, textFallbackCount, toolSource, content);

            if (!allParsed.isEmpty()) {
                // progress: LLM 选择了工具
                for (ParsedToolCall parsed : allParsed) {
                    progressSink.onProgress(AgentProgressEvent.toolSelected(
                            toolRound, maxToolRounds, parsed.tool(), buildParamsSummary(parsed.args())));
                }
                ...
                if (finishActionIdx >= 0 && finishActionIdx < allParsed.size() - 1) {
                    ...
                    progressSink.onProgress(
                            AgentProgressEvent.finishRejected(toolRound, maxToolRounds, "FINISH_ACTION_NOT_LAST"));
                    ...
                    trace.add(new MessageTrace("system", "finish_rejected", "FINISH_ACTION_NOT_LAST"));
                    log.debug("[ToolLoop] finish_action not last in round={}, tools={}", toolRound, toolsList);
                    toolRound++;
                    continue;
                }
                ...
                for (ParsedToolCall parsed : allParsed) {
                    ...
                    ToolLoopDebug.logToolCall(
                            log,
                            "runToolLoop",
                            toolRound,
                            parsed.tool(),
                            parsed.args().toString(),
                            callSource);
                    ...
                    if (execDecision.decision() == ToolExecutionDecisionType.REJECT) {
                        String reprompt =
                                executionPolicy.buildRejectionReprompt(parsed.tool(), execDecision, routeDecision);
                    ...
                    if (execDecision.decision() == ToolExecutionDecisionType.NEED_CONFIRMATION) {
                        if (permissionGate == null) {
                            ...
                            messages.add(LlmMessage.user(rejectMsg));
                            trace.add(new MessageTrace("system", "tool_rejected", "NO_PERMISSION_GATE"));
                            progressSink.onProgress(AgentProgressEvent.toolFailed(
                                    toolRound,
                                    maxToolRounds,
                                    parsed.tool(),
                        ...
                        progressSink.onProgress(
                                AgentProgressEvent.awaitingToolConfirmation(toolRound, maxToolRounds, parsed.tool()));

                        ToolConfirmationRequest confirmReq = new ToolConfirmationRequest(
                                parsed.tool(),
                                execDecision.category(),
                                execDecision.reason(),
                                parsed.args(),
                                contextMeta != null ? contextMeta.activeBranch() : null);
                        ConfirmationChoice choice = permissionGate.askConfirmation(confirmReq);
                        ToolLoopDebug.logToolPermissionDecision(log, "runToolLoop", toolRound, parsed.tool(), choice);

                        if (choice == ConfirmationChoice.DENY) {
                            String denyMsg = executionPolicy.buildDenyStopMessage(parsed.tool());
                            messages.add(LlmMessage.user(denyMsg));
                            trace.add(new MessageTrace(
                                    "system", "tool_denied", "tool=" + parsed.tool() + " choice=DENY"));
                            finalText = denyMsg;
                            break;
                        }
                    ...
                    if (result.success()) {
                        progressSink.onProgress(AgentProgressEvent.toolSuccess(
                                toolRound, maxToolRounds, parsed.tool(), buildResultSummary(parsed.tool(), result)));
                    } else {
                        progressSink.onProgress(
                                AgentProgressEvent.toolFailed(toolRound, maxToolRounds, parsed.tool(), result.error()));
                    }

                    String toolFeedback = buildToolResultFeedback(parsed.tool(), result);
                    messages.add(LlmMessage.user(toolFeedback));
                    trace.add(new MessageTrace(
                            "tool", "tool_result", "tool=" + parsed.tool() + " success=" + result.success()));
                    ToolLoopDebug.logToolResult(
                            log,
                            "runToolLoop",
                            toolRound,
                            parsed.tool(),
                ...
                if (finishActionSeen && finishMessage != null) {
                    ...
                    if (finishSummary != null && !finishSummary.isBlank()) {
                        progressSink.onProgress(
                                AgentProgressEvent.publicMessage("\033[34m" + finishSummary + "\033[0m"));
                    }

                    ToolLoopDebug.logFinishAction(
                            log,
                            "runToolLoop",
                            toolRound,
                            finishStatus,
                    ...
                    if (validationError != null) {
                        ...
                        ToolLoopDebug.logFinishAccepted(
                                log,
                                "runToolLoop",
                                toolRound,
                                false,
            ...
            if (toolSource == ToolLoopDebug.ToolCallSource.INVALID_TOOL_INTENT) {
                progressSink.onProgress(AgentProgressEvent.invalidBracketIntent(toolRound, maxToolRounds));
                consecutiveInvalidToolIntent++;
                ToolLoopDebug.logInvalidToolIntent(
                        log, "runToolLoop", toolRound, consecutiveInvalidToolIntent, content);

                if (consecutiveInvalidToolIntent >= 2) {
                    String errMsg = ToolLoopDebug.invalidToolIntentAbortError(consecutiveInvalidToolIntent);
                    trace.add(new MessageTrace("system", "error", errMsg));
                    ToolLoopDebug.logNoToolAbort(
                            log,
                            "runToolLoop",
                            toolRound,
                    ...
                    return new ChatResult(
                            ...
                            toolCalls,
                            trace,
                            "Agent produced invalid tool intent for " + consecutiveInvalidToolIntent
                                    + " consecutive rounds");
                }

                String reprompt = ToolLoopDebug.buildInvalidToolIntentReprompt();
                messages.add(LlmMessage.user(reprompt));
                trace.add(new MessageTrace("system", "invalid_tool_intent", "reprompting with correction"));
                toolRound++;
                continue;
            }
            ...
            consecutiveInvalidToolIntent = 0;
            {
                ...
                // 有意义的纯文本 → 直接接受，本轮结束
                if (content != null && !content.isBlank() && isMeaningfulAssistantContent(content)) {
                    finalText = content;
                    break;
                }
                ...
                if (consecutiveNoToolRounds >= 3) {
                    ...
                    ToolLoopDebug.logNoToolAbort(
                            ...
                            "runToolLoop",
                            toolRound,
                            "consecutive_no_tool_rounds_exceeded",
                            consecutiveNoToolRounds);
                    ...
                    return new ChatResult(
                            ...
                            toolCalls,
                            trace,
                            "Agent produced no tool calls for " + consecutiveNoToolRounds + " consecutive rounds");
                }
        ...
        if (finalText == null) {
            String errMsg = "[系统错误] Agent 未调用 finish_action，本轮未能正常结束。" + "请重试或检查 prompt/tool loop。";
            trace.add(new MessageTrace("system", "error", errMsg));
    ...
    /** 公共 tool-loop（sim 模式）。 */
    private SimResult runSimToolLoop(
            List<LlmMessage> messages,
            List<MessageTrace> trace,
            List<ToolCallRecord> toolCalls,
            AgentContextMeta contextMeta,
        ...
        while (toolRound <= maxToolRounds) {
            ...
            // task brief
            String lastToolName =
                    !toolCalls.isEmpty() ? toolCalls.get(toolCalls.size() - 1).tool() : null;
            ...
            progressSink.onProgress(AgentProgressEvent.contextLoaded(
                    toolRound, maxToolRounds, totalMessageChars + toolsJsonChars, toolDefs.size()));
            ...
            ToolLoopDebug.logLlmResult(log, "runSimToolLoop", toolRound, content, apiToolCallCount, finishReason);
            ...
            if (!allParsed.isEmpty()) {
                ...
                if (finishActionSeen && finishMessage != null) {
                    ...
                    ToolLoopDebug.logFinishAccepted(log, "runSimToolLoop", toolRound, true, null, null, null, null);
            ...
            if (toolSource == ToolLoopDebug.ToolCallSource.INVALID_TOOL_INTENT) {
                consecutiveInvalidToolIntent++;
                ToolLoopDebug.logInvalidToolIntent(
                        log, "runSimToolLoop", toolRound, consecutiveInvalidToolIntent, content);
                ...
                if (consecutiveInvalidToolIntent >= 2) {
                    ...
                    ToolLoopDebug.logNoToolAbort(
                            ...
                            "consecutive_invalid_tool_intent_exceeded",
                            consecutiveInvalidToolIntent);
                    ToolLoopDebug.logFinalText(log, "runSimToolLoop", "invalid_tool_intent_abort", errMsg);
