...
public final class BootstrapIntentParser {
    ...
    private static final String[] INIT_PREFIXES = {
        "初始化根节点：", "初始化根节点:", "初始化世界：", "初始化世界:", "创建第一个根节点：", "创建第一个根节点:", "init root:", "initialize root:",
    };
    ...
    /**
     * 快速检查用户输入是否应触发 bootstrap。
     *
     * @param userInput   原始用户输入
     * @param isDataEmpty dataRoot 是否严格为空
     * @return 如果应触发 bootstrap 则返回 {@code true}
     */
    ...
    /**
     * 获取 data 非空时拒绝自动 bootstrap 的提示消息。
     *
     * @return 提示消息字符串，指导用户使用 /root create 命令
     */
