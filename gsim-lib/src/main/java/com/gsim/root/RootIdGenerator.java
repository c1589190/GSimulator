...
public final class RootIdGenerator {
     ...
     * <p>允许的字符：字母（大小写）、数字、点、下划线、短横线。</p>
     *
     * @param rootId 待校验的 rootId
     * @return 如果符合规则则返回 {@code true}
     */
    ...
    /**
     * 从用户提供的 rootId 清洗或生成。如果显式 rootId 无效，则从 content 生成。
     *
     * @param explicitRootId 用户显式指定的 rootId，可为 {@code null} 或空白
     * @param content        用于生成 rootId 的内容（当显式 rootId 无效时使用）
     * @return 有效的 rootId
     */
     ...
     *
     * @param title          世界名称标题
     * @param initialContent 初始设定内容
     * @return 格式化的 Markdown 文本，包含世界名称、起始时间、初始设定等章节
     */
     ...
     * <p>按句号、逗号、换行、分号、英文句点等分隔符截取，超长时截断至 50 字符。</p>
     *
     * @param content 原始内容
     * @return 提取的标题，如果内容为空则返回 {@code "未命名世界"}
     */
