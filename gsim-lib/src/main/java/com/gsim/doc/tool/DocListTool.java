...
public final class DocListTool implements AgentTool {
                        ...
                        Map.of(
                                "type",
                                        Map.of(
                                                ...
                                                "string",
                                                "description",
                                                "按文档类型过滤: character, skill, world_state, template, context, rule"),
                                "tag", Map.of("type", "string", "description", "按标签过滤（可选）")),
                "required", List.of());
    }
    ...
    public ToolResult execute(ToolCall call) {
        ...
        if (docs.isEmpty()) {
            return ToolResult.ok(name(), List.of(new ToolResult.Item("(empty)", "docs", "暂无文档", 0)));
        }
