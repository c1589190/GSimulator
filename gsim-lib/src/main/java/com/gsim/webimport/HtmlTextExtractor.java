...
public class HtmlTextExtractor {
     ...
     *
     * @param html HTML 字符串
     * @return 提取到的标题，优先使用 &lt;title&gt; 标签，其次使用 h1，未找到则返回空字符串
     */
    ...
    public String extractText(String html, String url) {
        try {
            ...
            doc.select("script, style, nav, footer, header, aside, "
                            ...
                            + ".comment, .comments, .footer, .header, noscript, iframe")
                    .remove();
     ...
     *
     * @param html    HTML 字符串
     * @param baseUri 基础 URI，用于解析相对路径
     * @return 提取到的绝对 URL 列表
     */
    ...
    private void extractListItems(Element list, StringBuilder sb, String prefix) {
        for (Element li : list.children()) {
            if ("li".equals(li.tagName().toLowerCase(Locale.ROOT))) {
    ...
    private void extractTable(Element table, StringBuilder sb) {
        ...
        for (Element row : rows) {
            ...
            String rowText = cells.stream().map(c -> c.text().trim()).collect(Collectors.joining(" | "));
