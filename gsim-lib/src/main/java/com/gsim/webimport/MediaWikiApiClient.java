...
public class MediaWikiApiClient {
     ...
     *
     * @param pageId 页面 ID
     * @return ApiPageResult 包含解析后的页面内容
     * @throws IOException 如果 API 请求失败或返回错误
     */
    public ApiPageResult getPageByPageId(long pageId) throws IOException {
        String url = apiUrl + "?action=parse&pageid=" + pageId + "&prop=text|categories|links&format=json";
    ...
    public ApiPageResult getPageByTitle(String title) throws IOException {
        ...
        String url = apiUrl + "?action=parse&page=" + encodedTitle + "&prop=text|categories|links&format=json";
        return executeParse(url);
    }
     ...
     *
     * @param curid 页面 curid
     * @return ApiPageResult 包含解析后的页面内容
     * @throws IOException 如果 API 请求失败或返回错误
     */
    public ApiPageResult getPageByCurid(long curid) throws IOException {
        String url = apiUrl + "?action=parse&curid=" + curid + "&prop=text|categories|links&format=json";
        return executeParse(url);
    }
     ...
     * 获取指定命名空间的所有页面列表。
     *
     * @param namespace 命名空间编号
     * @param limit     单次返回的最大页面数
     * @return 页面标题列表
     * @throws IOException 如果 API 请求失败或返回错误
    ...
    public List<String> getAllPages(int namespace, int limit) throws IOException {
        String url = apiUrl + "?action=query&list=allpages" + "&apnamespace="
                + namespace + "&aplimit="
                + Math.min(limit, 500) + "&format=json";
     ...
     * @return 搜索结果列表（每个结果包含标题、pageId、摘要和字数）
     * @throws IOException 如果 API 请求失败或返回错误
     */
    public List<SearchResult> search(String query, int limit) throws IOException {
        String url = apiUrl + "?action=query&list=search" + "&srsearch="
                + URLEncoder.encode(query, StandardCharsets.UTF_8) + "&srlimit="
                + Math.min(limit, 20) + "&format=json";
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            if (searchResults.isArray()) {
                for (JsonNode sr : searchResults) {
                    ...
                    String snippet =
                            sr.has("snippet") ? stripHtml(sr.get("snippet").asText()) : "";
     ...
     *
     * @param pageId 页面 ID
     * @return 页面摘要文本，如果获取失败则返回 null
     */
    public String getExtract(long pageId) {
        try {
            String url = apiUrl + "?action=query&prop=extracts" + "&exintro&explaintext"
                    + "&pageids="
                    + pageId + "&format=json";
    ...
    public String getExtractByTitle(String title) {
        try {
            String url = apiUrl + "?action=query&prop=extracts" + "&exintro&explaintext"
                    + "&titles="
                    + URLEncoder.encode(title, StandardCharsets.UTF_8) + "&format=json";
     ...
     * API 页面结果。
     */
    public record ApiPageResult(String title, long pageId, String html, List<String> links) {}

    /**
     * allpages API 结果。
     */
    public record AllPagesResult(List<String> titles, String continueToken) {}

    /**
     * 搜索结果。
     */
    public record SearchResult(String title, long pageId, String snippet, int wordCount) {}
}
