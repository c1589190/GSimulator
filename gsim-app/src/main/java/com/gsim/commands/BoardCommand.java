...
public final class BoardCommand {
    ...
    /**
     * 执行 /board 命令，根据子命令分发到对应处理逻辑。
     *
     * @param args 命令行参数列表，第一个元素为子命令（list / read / create / write / append）
     * @return 命令执行结果文本
     */
    public String execute(List<String> args) {
        ...
        return switch (sub) {
            ...
            case "read" -> args.size() < 2 ? "Usage: /board read <boardId>" : readBoard(args.get(1));
            case "create" -> args.size() < 2 ? "Usage: /board create <title>" : createBoard(joinArgs(args, 1));
    ...
    private String listBoards() {
        ...
        List<Document> matching =
                all.stream().filter(d -> d.id().startsWith(prefix)).toList();
    ...
    private String createBoard(String title) {
        ...
        try {
            ...
            return "已创建 board: `" + docId + "`\n" + "  Agent 可通过 doc_write(docId=\"" + docId
                    + "\", content=\"...\") 写入内容。";
        } catch (IOException e) {
