...
public final class TextEditTool implements AgentTool {
    ...
    public TextEditTool(Path worldsDir, String activeWorldId, Path importDir,
    ...
    public Map<String, Object> getParameters() {
        ...
        props.put("source", Map.of("type", "string",
                "description", "文本来源：@cache:id、@world:ref、@doc:id、@import:id 或裸文本"));
        props.put("select_lines", Map.of("type", "string",
                "description", "保留的行范围，如 \"1-6, 11-14, 20\"（可选）"));
        props.put("delete_lines", Map.of("type", "string",
                "description", "删除的行范围，如 \"7-10, 22\"（可选）"));
        props.put("insert_at", Map.of("type", "integer",
                "description", "插入位置行号 (0-based)（可选，与 insert_text 配合）"));
        props.put("insert_text", Map.of("type", "string",
                "description", "要插入的文本（可选，与 insert_at 配合）"));
        props.put("replace_spec", Map.of("type", "string",
                "description", "要替换的行范围，如 \"3-4\"（可选，与 replace_text 配合）"));
        props.put("replace_text", Map.of("type", "string",
                "description", "替换为的文本（可选，与 replace_spec 配合）"));
        props.put("replace_from", Map.of("type", "string",
                "description", "要替换的关键词，逗号分隔（可选）"));
        props.put("replace_to", Map.of("type", "string",
                "description", "替换为的文本，逗号分隔（可选）"));
        props.put("mask_kw", Map.of("type", "string",
                "description", "要遮蔽为 *** 的关键词，逗号分隔（可选）"));
        props.put("mask_lines_spec", Map.of("type", "string",
                "description", "整行遮蔽的行范围，如 \"8-9\"（可选）"));
    ...
    public ToolResult execute(ToolCall call) {
        ...
        return ToolResult.ok(NAME, List.of(new ToolResult.Item(
                "text_edit: " + sourceLabel + " → " + result.summary(),
                cacheId,
                sb.toString(),
                1.0)));
    }
