...
public class AgentsManager {
    ...
    public AgentsManager(AgentConfigStore configStore, AgentCacheStore cacheStore,
                         LlmProviderRegistry llmRegistry, ToolRegistry allTools,
                         AgentProgressSink rootSink, EventBus eventBus, String model,
                         Path worldsDir, Supplier<String> worldIdSupplier) {
     ...
     */
    public AgentInstance runAgent(String configId, String cacheId, String prompt,
        ...
        AgentInstance instance = new AgentInstance(
                instanceId, configId, sessionId, taskId, resolvedCacheId,
                parentInstanceId, prompt, AgentStatus.RUNNING, Instant.now(), null, null);
        instances.put(instanceId, instance);

        // 创建 AbstractAgent
        AgentConfig config = configStore.get(configId);
        if (config == null) {
            AgentInstance failedRef = instance.withStatus(AgentStatus.FAILED, Instant.now(),
                    "Unknown agent config: " + configId);
        ...
        // 发布 agent_started 事件
        publishEvent(sessionId, taskId, "agent_started", Map.of(
        ...
        Thread.startVirtualThread(() -> {
            try {
                ...
                if (result.success()) {
                    publishEvent(sessionId, taskId, "agent_result", Map.of(
                            "instanceId", instanceId,
                            "cacheId", resolvedCacheId,
                            "finalText", truncate(result.finalText(), 2000),
                            "totalToolCalls", String.valueOf(result.totalToolCalls())
                    ));
                    instances.put(instanceId, instance.withStatus(AgentStatus.DONE, Instant.now(), null));
                } else {
                    publishEvent(sessionId, taskId, "agent_error", Map.of(
                            "instanceId", instanceId,
                            "error", result.error() != null ? result.error() : "unknown"
                    ));
                    instances.put(instanceId, instance.withStatus(AgentStatus.FAILED, Instant.now(),
                            result.error()));
                }

                publishEvent(sessionId, taskId, "agent_done", Map.of(
                        "instanceId", instanceId,
        ...
        log.info("[AgentsManager] started {} (config={}, cache={}, parent={})",
                instanceId, configId, resolvedCacheId, parentInstanceId);
        return instance;
    }
    ...
    public boolean cancelAgent(String instanceId) {
        ...
        if (instance.status() == AgentStatus.RUNNING || instance.status() == AgentStatus.PENDING) {
            ...
            publishEvent(instance.sessionId(), instance.taskId(), "agent_done", Map.of(
                    "instanceId", instanceId, "status", "CANCELLED"));
    ...
    /** 列出 Agent（支持可选的 status/configId/parentInstanceId 过滤）。 */
    public List<AgentInstance> listAgents(String configId, AgentStatus status,
                                           String parentInstanceId) {
    ...
    /**
     * 阻塞等待 Agent 完成，返回结果。
     * 供 dispatch_sub_agent 工具在同步语义中使用。
     */
