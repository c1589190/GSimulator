...
public class AgentCachesApiHandler implements HttpHandler {
    ...
    private void handleList(HttpExchange exchange) throws IOException {
        ...
        data.put("caches", caches.stream().map(AgentCachesApiHandler::cacheInfoToMap).toList());
    ...
    private void handleCreate(HttpExchange exchange) throws IOException {
        ...
        BaseApiHandler.sendJson(exchange, 201,
                ApiResponse.ok("Cache created: " + session.sessionId(), data));
    }
    ...
    private void handleDelete(HttpExchange exchange, String cacheId) throws IOException {
        ...
        if (deleted) {
            BaseApiHandler.sendOk(exchange, "Cache deleted: " + cacheId,
                    Map.of("cacheId", cacheId, "action", "deleted"));
        } else {
            BaseApiHandler.sendError(exchange, 404, "Cache not found: " + cacheId);
        }
    }
