...
public class TimelineApiHandler implements HttpHandler {
    ...
    /**
     * 创建 TimelineApiHandler 实例。
     *
     * @param worldInfo 世界信息提供者
     * @param worldsDir 世界数据文件根目录
     * @param worldId   当前世界 ID 提供者
     */
    public TimelineApiHandler(Supplier<WorldInformation> worldInfo, Path worldsDir, Supplier<String> worldId) {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            } else if (path.startsWith("/timeline/nodes/")
                    && path.endsWith("/activate")
                    && "POST".equalsIgnoreCase(method)) {
                String nodeId = path.substring("/timeline/nodes/".length(), path.length() - "/activate".length());
    ...
    private void handleNodeDetail(HttpExchange exchange) throws IOException {
        ...
        if (nodeId == null || wi == null) {
            HandlerUtils.sendHtml(exchange, 200, "<div class='text-gray-500'>Node not found</div>");
            return;
        }

        NodeSnapshot node = wi.nodeById(nodeId);
        if (node == null) {
            HandlerUtils.sendHtml(exchange, 200, "<div class='text-gray-500'>Node not found: " + nodeId + "</div>");
            return;
        }
    ...
    private void handleActivateNode(HttpExchange exchange, String nodeId) throws IOException {
        ...
        if (wi == null || wi.nodeById(nodeId) == null) {
            HandlerUtils.sendJson(exchange, 404, Map.of("error", "Node not found in current world"));
            return;
        }
