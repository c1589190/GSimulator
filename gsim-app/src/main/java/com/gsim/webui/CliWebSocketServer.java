...
import com.gsim.core.event.CompositeAgentProgressSink;
import com.gsim.core.session.CliNodeRenderer;
import com.gsim.core.session.NodeStatus;
import com.gsim.core.session.NodeType;
import com.gsim.core.session.SessionNode;
import com.gsim.core.session.SessionNodeJsonRenderer;
import com.gsim.core.session.SessionNodeListener;
...
public class CliWebSocketServer {
    ...
    /**
     * 创建 CliWebSocketServer 实例。
     *
     * @param ctx           应用上下文
     * @param port          服务器监听端口
     * @param compositeSink 复合进度 Sink，用于实时推送 Agent 事件到 WebSocket
     */
    public CliWebSocketServer(ApplicationContext ctx, int port, CompositeAgentProgressSink compositeSink) {
        this.ctx = ctx;
        this.port = port;
     ...
     * 启动 WebSocket 服务器。
     *
     * @throws IOException 如果服务器套接字创建失败
     */
    ...
    private void handleCliClient(Socket client, InputStream in, OutputStream out) throws IOException {
        ...
        SessionNodeListener poolListener = new SessionNodeListener() {
            ...
            public void onNodePushed(SessionNode node) {
                ...
                if (node.type() == NodeType.LLM_STREAMING) {
                    try {
                        sendWs(out, renderer.startLlmStream());
                    } catch (IOException e) {
                    }
                }
            }
            ...
            public void onStatusChanged(SessionNode node, NodeStatus oldStatus, NodeStatus newStatus) {
                if (node.type() == NodeType.LLM_STREAMING && newStatus == NodeStatus.DONE) {
                    try {
                        sendWs(out, renderer.endLlmStream(true));
                    } catch (IOException e) {
                    }
                }
            }
        ...
        while (running && !client.isClosed()) {
            ...
            sessionPool.pushNode("cli", NodeType.USER_INPUT, Map.of("text", cmd, "source", "websocket"));
    ...
    private void handleChatClient(Socket client, InputStream in, OutputStream out, String sessionId)
            throws IOException {
        ...
        if (existingNodes != null && !existingNodes.isEmpty()) {
            String historyJson = SessionNodeJsonRenderer.buildMessage(
                    "history", SessionNodeJsonRenderer.renderHistory(existingNodes));
            ...
            for (SessionNode node : existingNodes) {
                if (node.type() == NodeType.LLM_STREAMING) {
                    ...
                    if (status == NodeStatus.STREAMING || status == NodeStatus.PENDING) {
                        String streamingJson = SessionNodeJsonRenderer.buildMessage(
                                "streamingState", Map.of("node", SessionNodeJsonRenderer.render(node)));
        ...
        SessionNodeListener poolListener = new SessionNodeListener() {
            ...
            public void onNodePushed(SessionNode node) {
                ...
                String json = SessionNodeJsonRenderer.buildMessage(
                        "nodePushed", Map.of("node", SessionNodeJsonRenderer.render(node)));
                try {
                    sendWs(out, json);
                } catch (IOException e) {
                }
            ...
            public void onNodeUpdated(SessionNode node, String key, Object newValue) {
                if (!sessionId.equals(node.sessionId())) return;
                String json = SessionNodeJsonRenderer.buildMessage(
                        "nodeUpdated", SessionNodeJsonRenderer.renderUpdate(node.nodeId(), key, newValue));
                try {
                    sendWs(out, json);
                } catch (IOException e) {
                }
