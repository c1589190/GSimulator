package com.gsim.agentsmanager.prompt;
...
class OrchestratorPromptNoLegacySimRunRulesTest {
    ...
    static {
        try (InputStream is = OrchestratorPromptNoLegacySimRunRulesTest.class
                .getClassLoader()
                .getResourceAsStream("gsim/prompts/orchestrator-system.md")) {
            if (is != null) {
    ...
    void noLegacySimInstruction() {
        assertFalse(promptContent.contains("/sim 时"), "不应有 /sim 独占指令，已改为统一 Agent 入口");
    }

    @Test
    @DisplayName("不包含旧 /run 独占指令")
    void noLegacyRunInstruction() {
        assertFalse(promptContent.contains("/run 时"), "不应有 /run 独占指令，已改为统一 Agent 入口");
    }

    @Test
    @DisplayName("声明 /sim 和 /run 已废弃")
    void declaresSimAndRunDeprecated() {
        assertTrue(promptContent.contains("/sim") || promptContent.contains("/run"), "应提到 /sim /run（以声明已废弃）");
        assertTrue(promptContent.contains("废弃"), "应明确声明 /sim 和 /run 已废弃");
    }

    @Test
    @DisplayName("不应要求只输出 JSON 工具调用（与统一 Agent 冲突）")
    void noJsonOnlyOutputRule() {
        assertFalse(promptContent.contains("不要输出其他文本，只输出 JSON 工具调用"), "不应有只输出 JSON 工具调用的独占规则");
    }

    @Test
    @DisplayName("应包含统一自然语言入口说明")
    void hasUnifiedNaturalLanguageEntry() {
        assertTrue(promptContent.contains("统一自然语言"), "应说明通过统一自然语言 Agent 入口工作");
    }

    @Test
    @DisplayName("应说明不需要工具时直接自然语言回答")
    void hasNaturalLanguageFallbackRule() {
        assertTrue(promptContent.contains("不需要工具时") || promptContent.contains("直接自然语言"), "应说明不需要工具时直接自然语言回答");
    }
    ...
    void hasFactInferenceHypothesisRule() {
        assertTrue(promptContent.contains("facts") || promptContent.contains("已知事实"), "应要求区分已知事实");
        assertTrue(promptContent.contains("inferences") || promptContent.contains("推断"), "应要求区分推断");
        assertTrue(promptContent.contains("hypotheses") || promptContent.contains("假设"), "应要求区分假设");
    }
    ...
    void noAskUserToSwitchSimRun() {
        assertTrue(
                promptContent.contains("不要要求用户改用 /sim") || promptContent.contains("不要要求用户改用/sim"),
    ...
    void hasWorldInfoQueryFormat() {
        assertTrue(promptContent.contains("nodeId:checkpointId:key"), "应包含 WorldInfo nodeId:checkpointId:key 查询格式说明");
    }

    @Test
    @DisplayName("应包含 query_element 和 write_element 工具说明")
    void hasWorldInfoToolDescriptions() {
        assertTrue(promptContent.contains("query_element"), "应包含 query_element 工具说明");
        assertTrue(promptContent.contains("write_element"), "应包含 write_element 工具说明");
    }
