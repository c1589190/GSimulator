...
public final class DocWriteTool implements AgentTool {
     ...
     *
     * @param store         文档存储
     * @param cacheManager  缓存管理器，用于解析 @cache: 引用
                        ...
                        Map.of(
                                ...
                                "mode",
                                        Map.of(
                                                ...
                                                "string",
                                                "description",
                                                "写入模式: replace, append, line_range。默认 replace"),
                                "line_start",
                                        Map.of(
                                                "type",
                                                "integer",
                                                "description",
                                                "行范围起始行 (0-based, 含)。仅 mode=line_range"),
                                "line_end",
                                        Map.of(
                                                "type",
                                                "integer",
                                                "description",
                                                "行范围结束行 (0-based, 不含)。仅 mode=line_range"),
    ...
    public ToolResult execute(ToolCall call) {
        ...
        boolean isBoard = old != null && old.type() == com.gsim.docslib.doc.DocType.BOARD;
        ...
        try {
            ...
            if (isBoard) {
                ...
                progressSink.onProgress(com.gsim.agentsmanager.event.AgentProgressEvent.publicMessage(
                        "\n📋 " + boardTitle + "\n" + content));
            }

            // 同时更新元数据
            if (!newTitle.isEmpty() || !tagsStr.isEmpty()) {
                List<String> tags = tagsStr.isEmpty() ? old.tags() : List.of(tagsStr.split("\\s*,\\s*"));
                store.updateMeta(docId, newTitle.isEmpty() ? old.title() : newTitle, tags);
            }
