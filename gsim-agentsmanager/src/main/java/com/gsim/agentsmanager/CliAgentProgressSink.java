...
public class CliAgentProgressSink implements AgentProgressSink {
     ...
     *
     * @param terminal JLine3 Terminal 实例
     * @return 绑定到 JLine 终端输出的 CliAgentProgressSink
     */
    public static CliAgentProgressSink fromJlineTerminal(org.jline.terminal.Terminal terminal) {
        return new CliAgentProgressSink(new PrintStream(terminal.output(), true), true);
    }
     ...
     *
     * @param event 进度事件
     * @return 格式化后的状态行字符串，null 表示该事件应在终端静默
     */
    static String format(AgentProgressEvent event) {
        ...
        return switch (event.phase()) {
            ...
            case AgentProgressEvent.TOOL_FAILED -> {
                ...
                yield ANSI_RED + "❌ " + tool + ANSI_RESET + (error.isBlank() ? "" : " — " + error);
            }
            case AgentProgressEvent.LLM_STREAM_FAILED -> {
                String error = event.meta().getOrDefault("error", "未知错误");
                yield "[Agent] LLM 流式输出失败: " + error;
            }
            case AgentProgressEvent.ABORTED -> "[Agent] " + event.detail();
            ...
            case AgentProgressEvent.TOOL_SUCCESS -> {
                ...
                // ── 公开消息/推演内容（终端可见） ──
            case AgentProgressEvent.AGENT_PUBLIC_MESSAGE -> event.detail();

                // ── finish_action 拒绝（终端可见，用户需知晓） ──
            case AgentProgressEvent.FINISH_ACTION_REJECTED -> "[Agent] finish_action 被拒绝："
    ...
    /**
     * 将 rejectReason 错误码转换为人类可读的中文描述。
     *
     * @param reasonCode 拒绝原因代码
     * @return 中文描述文本
     */
    static String reasonText(String reasonCode) {
        if (reasonCode == null) return "未知原因";
        return switch (reasonCode) {
            case "FINISH_ACTION_WITH_OTHER_TOOLS" -> "与其他工具同轮混用，要求模型先单独执行工具。";
            case "CLAIM_WITHOUT_SUPPORTING_TOOL_RESULT" -> "声称了未经真实工具执行支持的结果。";
            default -> reasonCode;
        };
    }
    ...
    private static final java.util.Set<String> WRITE_TOOLS =
            java.util.Set.of("write_element", "node_create", "create_checkpoint");

    /**
     * 格式化工具调用行：🔧 toolName key=val ...
     *
     * @param tool          工具名称
     * @param paramsSummary 参数摘要
     * @return 格式化后的工具调用字符串
     */
    ...
    private void handleSubAgentEvent(AgentProgressEvent event, String agentId) {
        switch (event.phase()) {
            ...
            case AgentProgressEvent.TOOL_FAILED -> {
                String tool = event.meta().getOrDefault("tool", "");
                String error = event.meta().getOrDefault("error", "");
                out.println("[SubAgent " + agentId + "] tool failed: " + tool + (error.isBlank() ? "" : " — " + error));
            }
