...
public class ActivateToolGroupsTool implements AgentTool {
    ...
    public ToolResult execute(ToolCall call) {
        ...
        try {
            ...
            return ToolResult.fail(
                    NAME, "groups 格式无效：" + e.getMessage() + "。请使用 JSON 字符串数组格式，例如 [\"player_action\",\"knowledge\"]。");
        }
        ...
        if (!alreadyActive.isEmpty()) {
            ...
            resultText
                    .append("已处于激活状态: ")
                    .append(String.join(", ", alreadyActive))
                    .append("。");
        }
        if (!unknown.isEmpty()) {
            if (!resultText.isEmpty()) resultText.append(" ");
    ...
    /**
     * 解析 groups 参数，支持标准 JSON 数组字符串或 LLM 可能生成的变体格式。
     *
     * @param raw 原始 groups 参数字符串，如 {@code "[\"player_action\",\"knowledge\"]"} 或 {@code "a,b,c"}
     * @return 解析后的组 key 列表；若输入为空数组或空字符串则返回空列表
     */
