...
public class CollectSubAgentResultsTool implements AgentTool {
    ...
    public Map<String, Object> getParameters() {
        return ToolDef.strictSchema(Map.of(), List.of());
    }

    @Override
    public ToolResult execute(ToolCall call) {
        if (runningSubAgents.isEmpty()) {
            return ToolResult.fail(NAME, "当前没有正在运行的子代理。请先调用 dispatch_sub_agent 创建子代理。");
        }
        ...
        sb.append("---\n");
        sb.append("汇总: ").append(successCount).append(" 成功, ").append(failCount).append(" 失败");
        ...
        log.info("[CollectSubAgent] completed: {} success, {} fail, {} total", successCount, failCount, totalCount);
