...
public final class GsimSearchTool implements AgentTool {
    ...
    public GsimSearchTool(SearchToolContext ctx) {
        this.ctx = ctx;
        this.worldSource = new WorldSearchSource(ctx);
        this.regionSource = new RegionSearchSource(ctx);
        this.hexSource = new HexSearchSource(ctx);
     ...
     *
     * <p>worldsDir 为 null（SearchToolContext 以兼容四参构造）时无法构造
