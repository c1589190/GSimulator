 ...
 * 在钩子内读取 {@code type}/{@code tag} 参数并委托给私有核心方法；两参抽象方法以
...
public final class GsimSearchDocTool extends AbstractSearchTool {
    ...
    protected List<SearchEntry> buildEntries(String worldId, String effectiveNodeId, ToolCall call) {
        ...
        return doBuild(call.param("type"), call.param("tag"));
    }
    ...
    /**
     * 核心语料构造：直读 DocStore（新鲜），每条文档生成一条 {@link SearchEntry}。
     *
     * <p>命名避开 {@code buildEntries}（两个同名 (String,String) 方法会因擦除冲突）——
     * 2 参 {@link #buildEntries(String, String)} 是共享契约的抽象覆写，必须保留原签名。
     *
     * @param typeParam 可选的 DocType key 过滤（null/空白表示不过滤；未知 key 落入 OTHER）
     * @param tagParam  可选的标签过滤（null/空白表示不过滤）
     * @return 搜索语料列表
     */
    private List<SearchEntry> doBuild(String typeParam, String tagParam) {
