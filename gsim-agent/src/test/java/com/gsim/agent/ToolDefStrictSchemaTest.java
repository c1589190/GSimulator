...
class ToolDefStrictSchemaTest {
    ...
    void finishActionToolDefStrictSchema() {
        ...
        assertEquals("object", params.get("type"), "top-level type 应为 object");
        assertEquals(false, params.get("additionalProperties"), "additionalProperties 应为 false");

        @SuppressWarnings("unchecked")
        java.util.List<String> required = (java.util.List<String>) params.get("required");
        assertNotNull(required, "required 数组不应为 null");
        assertTrue(required.contains("message"), "required 应包含 message，实际: " + required);

        @SuppressWarnings("unchecked")
        Map<String, Map<String, Object>> properties = (Map<String, Map<String, Object>>) params.get("properties");
        assertNotNull(properties, "properties 不应为 null");
        assertTrue(properties.containsKey("message"), "properties 应包含 message key");

        Map<String, Object> messageProp = properties.get("message");
        assertEquals("string", messageProp.get("type"), "message property type 应为 string");
    }
    ...
    void toolDefPreservesSchemaFromAgentTool() {
        ...
        assertEquals(false, td.parameters().get("additionalProperties"), "ToolDef 应保留 additionalProperties=false");
    }
    ...
    void toolDefWithoutParametersUsesDefaultOpenSchema() {
        ...
        assertEquals(
                true, td.parameters().get("additionalProperties"), "默认 open schema 的 additionalProperties 应为 true");
    }
}
