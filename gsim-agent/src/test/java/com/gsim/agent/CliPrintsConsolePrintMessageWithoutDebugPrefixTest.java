...
class CliPrintsConsolePrintMessageWithoutDebugPrefixTest {
    ...
    void agentPublicMessageDirectOutput() {
        ...
        // 不应包含 [Agent] 前缀
        assertFalse(formatted.contains("[Agent]"), "Should NOT contain [Agent] debug prefix: " + formatted);
        // 应包含正文
        assertTrue(formatted.contains("报名表"), "Should contain the actual message content: " + formatted);
        assertTrue(formatted.contains("| 姓名 |"), "Should contain the table content: " + formatted);
    }
    ...
    void preservesCopyableTableContent() {
        ...
        assertEquals(longMessage, formatted, "The output should be exactly the message text, unmodified");
    }
    ...
    void errorEventsStillHaveAgentPrefix() {
        ...
        assertTrue(formatted.contains("bad_tool"), "Error events should include tool name: " + formatted);
        assertTrue(formatted.contains("some error"), "Error events should include error message: " + formatted);
        assertTrue(formatted.contains("❌"), "Error events should have ❌ marker: " + formatted);
    }
}
