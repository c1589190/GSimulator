...
class ConsoleEventSinkTest {
    ...
    void shouldDisplayCommandStarted() {
        sink.accept(GSimEvent.of("default", "command_started", Map.of("command", "/status")));
    ...
    void shouldDisplayCommandDoneAndError() {
        ...
        sink.accept(GSimEvent.of("default", "command_error", Map.of("error", "something wrong")));
        assertTrue(sw.toString().contains("[✗]"));
    ...
    void shouldStreamLlmDelta() {
        sink.accept(GSimEvent.of("default", "llm_delta", Map.of("content", "Hello")));
        sink.accept(GSimEvent.of("default", "llm_delta", Map.of("content", " world")));
        String output = sw.toString();
    ...
    void shouldPrefixReasoningDelta() {
        sink.accept(GSimEvent.of("default", "llm_reasoning_delta", Map.of("content", "thinking step 1...")));
        String output = sw.toString();
    ...
    void shouldBreakAfterReasoning() {
        sink.accept(GSimEvent.of("default", "llm_reasoning_delta", Map.of("content", "think...")));
        sink.accept(GSimEvent.of("default", "llm_delta", Map.of("content", "output")));
    ...
    void shouldDisplayToolEvents() {
        sink.accept(GSimEvent.of("default", "tool_started", Map.of("tool", "searchdb", "message", "正在查询")));
        sink.accept(GSimEvent.of("default", "tool_done", Map.of("tool", "searchdb", "count", 5)));
        String output = sw.toString();
        assertTrue(output.contains("[🔧]"));
    ...
    void shouldDisplayRunStage() {
        sink.accept(GSimEvent.of("default", "run_stage", Map.of("stage", "analyze_actions", "message", "正在分析")));
        String output = sw.toString();
        assertTrue(output.contains("[…]"));
    ...
    void shouldDisplayImportProgress() {
        sink.accept(GSimEvent.of("default", "import_progress", Map.of("message", "Starting import...")));
        String output = sw.toString();
        assertTrue(output.contains("[📥]"));
