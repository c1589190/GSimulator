...
class SseWriterTest {
    ...
    void shouldOutputSseFormat() {
        ...
        GSimEvent event = GSimEvent.of("default", "task-001", "command_started", Map.of("command", "/status"));
        sink.accept(event);
    ...
    void shouldIncludeTaskId() {
        ...
        GSimEvent event = GSimEvent.of("default", "task-123", "llm_delta", Map.of("text", "hello"));
        sink.accept(event);
    ...
    void shouldContainRequiredFields() {
        GSimEvent event = GSimEvent.of("s1", "t1", "run_stage", Map.of("stage", "analyze", "message", "analyzing..."));
