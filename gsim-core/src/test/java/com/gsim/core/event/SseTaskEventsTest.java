...
class SseTaskEventsTest {
    ...
    void sseFormatShouldBeCorrect() throws Exception {
        ...
        sink.accept(GSimEvent.of("default", "task-001", "command_started", Map.of("command", "/status")));
    ...
    void sseShouldContainSessionAndTaskId() throws Exception {
        ...
        sink.accept(GSimEvent.of("s1", "task-abc", "result", Map.of("success", true)));
    ...
    void eventSequenceShouldBeComplete() throws Exception {
        ...
        sink.accept(GSimEvent.of(sessionId, taskId, "command_started", Map.of("command", "/status")));
        sink.accept(GSimEvent.of(sessionId, taskId, "log", Map.of("message", "Running /status")));
        sink.accept(GSimEvent.of(sessionId, taskId, "result", Map.of("success", true)));
    ...
    void toSseJsonShouldSerializeCorrectly() {
        var event = GSimEvent.of("s1", "task-1", "my_type", Map.of("key1", "value1", "key2", 42));
