...
class SessionPoolBridgeTest {
    ...
    void toolFailedMarksError() {
        ...
        List<SessionNode> toolNodes =
                nodes.stream().filter(n -> n.type() == NodeType.TOOL_CALL).toList();
    ...
    void clearSessionCleansBridge() {
        ...
        assertDoesNotThrow(() -> bridge.onProgress(AgentProgressEvent.llmContentDelta("s1", "新的流")));
    }
