...
class RealApiStreamingTest {
    ...
    private static final String XFYUN_KEY = "9aadfef086e5ed90207b76acadbb256b:NmRkMzJiZTM2NWEwMDI5NWUzYmJiNjQy";
    private static final String XFYUN_MODEL = "astron-code-latest";

    private static final String SF_URL = "https://api.siliconflow.cn/v1";
    private static final String SF_KEY = "sk-nqndfqwmywnhietyuivzlfpmyjongcrpxplrwwvueiwpcsry";
    ...
    private static final String DS_KEY = "sk-408b8057faaa442a8b5e86cd20e2eb8e";
                    ...
                    Map.of(
                            "status",
                                    Map.of(
                                            ...
                                            "string",
                                            "description",
                                            "任务状态：success 或 failure",
                                            "enum",
                                            List.of("success", "failure")),
                            "message", Map.of("type", "string", "description", "给用户的最终消息")),
    ...
    void xfyunAutoToolChoice() throws Exception {
        ...
        LlmRequest request =
                buildRequest(List.of(LlmMessage.user("请调用 finish_action 工具。status=success, message=流式测试完成")), "auto");
        ...
        assertTrue(pool.getToolCalls().size() > 0 || !pool.getContent().isBlank(), "至少应有 tool_calls 或 content");
        assertFalse(pool.getContent().contains("null"), "content 不应是字面量 null");
    ...
    void xfyunForcedToolChoice() throws Exception {
        ...
        assertTrue(pool.isComplete(), "流式应标记完成");
        assertTrue(!pool.getToolCalls().isEmpty() || !pool.getContent().isBlank(), "至少应有 tool_calls 或 content");
        mgr.close();
    }
    ...
    void siliconFlowAutoToolChoice() throws Exception {
        ...
        LlmRequest request = buildRequest(
                List.of(LlmMessage.user("请调用 finish_action 工具。status=success, message=SiliconFlow流式测试完成")), "auto");
        ...
        assertTrue(pool.isComplete(), "流式应标记完成");
        assertTrue(!pool.getContent().isBlank() || !pool.getToolCalls().isEmpty(), "至少应有 content 或 tool_calls");
        assertFalse(pool.getContent().contains("null"), "content 不应是字面量 null");
    ...
    void deepseekAutoToolChoice() throws Exception {
        ...
        LlmRequest request = buildRequest(
                List.of(LlmMessage.user("请调用 finish_action 工具。status=success, message=DeepSeek流式测试完成")), "auto");
    ...
    void allApisDeliverEventsToPool() throws Exception {
        ...
        for (var api : apis) {
            ...
            LlmRequest request = buildRequest(
                    List.of(LlmMessage.user("请简单回复「你好」，然后调用 finish_action 工具，status=success，message=完成")), "auto");
            ...
            System.out.printf(
                    "[%s] CONTENT=%d REASONING=%d TOOL_CALL=%d%n",
                    api.name(), contentEvents, reasoningEvents, toolCallEvents);

            // 至少应有 content 事件或 tool_call 事件（不能是空池）
            assertTrue(contentEvents > 0 || toolCallEvents > 0, api.name() + " 流式池必须收到至少一种事件（CONTENT 或 TOOL_CALL）");

            assertFalse(pool.getContent().contains("null"), api.name() + " content 不应是字面量 null");
            mgr.close();
        }
    }
}
