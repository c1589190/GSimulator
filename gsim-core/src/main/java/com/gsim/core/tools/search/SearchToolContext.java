...
public record SearchToolContext(
    ...
    /**
     * 兼容构造（原始四组件语义）：路径组件为 null —— 地址解析（{@code @world:}/
     * {@code @cache:}/{@code @import:}/裸引用）不可用，gsim_search 自动回退关键词模式；
     * {@code @doc:} 解析仍可用（仅需 docStore）。
     */
    public SearchToolContext(Supplier<WorldInformation> wiSupplier, DocStore docStore, ResolverRegistry registry) {
        this(wiSupplier, docStore, registry, null, null, null);
    }
}
