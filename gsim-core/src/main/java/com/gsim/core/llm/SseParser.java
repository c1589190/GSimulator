...
class SseParser {
    ...
    void parse(BufferedReader reader) throws IOException {
        ...
        } else if (firstLine.startsWith("event:") || firstLine.startsWith("id:") || firstLine.startsWith("retry:")) {
    ...
    private void finishStream() {
        if (dataLinesProcessed == 0) {
            ...
            log.info(
                    "[SSE] stream done: {} data lines, contentChars={} reasoningChars={}",
                    dataLinesProcessed,
                    pool.getContent().length(),
                    pool.getReasoning().length());
        }
        pool.onComplete(new LlmResult(
                pool.getContent(), pool.getReasoning(), model, 0, true, null, pool.getToolCalls(), "stop", false));
    }
        ...
        try {
            ...
            if (!toolCalls.isEmpty()) {
                ...
                result = new LlmResult(
                        content, reasoning, model, tokensUsed, true, null, List.of(), finishReason, false);
            }
