...
public final class SessionPoolBridge implements AgentProgressSink {
    ...
    public void onProgress(AgentProgressEvent event) {
        ...
        try {
            ...
            pool.pushNode(
                    ...
                    Map.of(
                            "message",
                            "SessionPoolBridge error: " + e.getMessage(),
                            "error",
        ...
        switch (event.phase()) {
            ...
            case AgentProgressEvent.LLM_REASONING_DELTA -> {
                ...
                if (nodeId != null) {
                    pool.updateNode(
                            nodeId,
                            "reasoning",
                            pool.nodeById(nodeId).payload().getOrDefault("reasoning", "") + event.detail());
                }
            }
            ...
            case AgentProgressEvent.TOOL_FAILED -> {
                ...
                if (error != null && !error.isBlank()) {
                    pool.pushNode(sessionId, NodeType.SYSTEM, Map.of("message", "工具失败: " + toolName, "error", error));
                }
            }
            ...
            case AgentProgressEvent.FINISH_ACTION_ACCEPTED -> {
                pool.pushNode(sessionId, NodeType.SYSTEM, Map.of("message", "finish_action 通过，本轮结束。"));
            }
            case AgentProgressEvent.FINISH_ACTION_REJECTED -> {
                ...
                pool.pushNode(
                        sessionId,
                        NodeType.SYSTEM,
                        Map.of("message", "finish_action 被拒绝: " + (reason != null ? reason : "")));
            }
            case AgentProgressEvent.ABORTED -> {
                pool.pushNode(sessionId, NodeType.SYSTEM, Map.of("message", "中止: " + event.detail()));
            }
    ...
    private void markLatestToolCallDone(String toolName, String result, AgentProgressEvent event) {
        ...
        SessionNode node = pool.nodeById(nodeId);
        if (node == null) return;
        if (node.payload().get("status") == NodeStatus.DONE || node.payload().get("status") == NodeStatus.ERROR) return;
        node.payload().put("result", result);
        node.payload().put("detail", event.detail());
        pool.transitionStatus(nodeId, "failed".equals(result) ? NodeStatus.ERROR : NodeStatus.DONE);
    }
    ...
    /**
     * 清理该 session 相关的所有 bridge 映射。
     *
     * <p>清除 streamNodes 映射和 lastToolCallNodeId，不操作 SessionPool 本身。
     */
