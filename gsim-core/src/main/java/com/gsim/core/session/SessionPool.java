...
public class SessionPool {
     ...
     *
     * @param sessionId 目标会话 ID
     * @param parentId  父节点 ID，用于构建因果链
     * @param type      节点类型
     * @param payload   负载数据
     * @return 新创建的 SessionNode
     */
    public SessionNode pushNode(String sessionId, String parentId, NodeType type, Map<String, Object> payload) {
        String nodeId = IdGenerator.generate("node");
        SessionNode node = new SessionNode(nodeId, sessionId, parentId, type, Instant.now(), payload);
     ...
     *
     * <p>安全处理 content 可能是 StringBuilder（流式写入中）或 String（已完成）的情况。
     *
     * @param nodeId 节点 ID
     * @param delta  要追加的字符串片段
     */
     ...
     *
     * @param nodeId 节点 ID
     * @return content 字符串，节点不存在或 content 为空时返回 ""
     */
    ...
    public void transitionStatus(String nodeId, NodeStatus newStatus) {
        ...
        notifyListeners(node.sessionId(), l -> l.onStatusChanged(node, oldStatus, newStatus));
    }
     ...
     *
     * @param sessionId 会话 ID
     * @return 下一个 USER_INPUT 节点的 CompletableFuture
    ...
    public CompletableFuture<SessionNode> waitForUserInput(String sessionId) {
        ...
        if (old != null) {
            throw new IllegalStateException("Another waiter already exists for session: " + sessionId);
        }
        return future;
    }
    ...
    private void notifyListeners(String sessionId, java.util.function.Consumer<SessionNodeListener> action) {
        List<SessionNodeListener> list = listeners.get(sessionId);
        if (list != null) {
    ...
    /**
     * 移除指定 session 的所有节点及相关监听器和等待者（清理用）。
     *
    ...
    public void clearSession(String sessionId) {
        ...
        CompletableFuture<SessionNode> waiter = waiters.remove(sessionId);
        if (waiter != null) {
            waiter.completeExceptionally(new IllegalStateException("Session cleared while waiting: " + sessionId));
        }
    }
