...
public class ContourQueryEngine {
     ...
     */
    public com.gsimap.map.MapData materialize(int minQ, int maxQ, int minR, int maxR) {
        var hexes = new LinkedHashMap<String, com.gsimap.map.MapData.HexCell>();
        for (int q = minQ; q <= maxQ; q++) {
            for (int r = minR; r <= maxR; r++) {
                ...
                hexes.put(
                        q + "_" + r,
                        new com.gsimap.map.MapData.HexCell(ts.color, ts.terrain, null, null, "", 0, Map.of()));
            }
        }
        return new com.gsimap.map.MapData(
                30,
                false,
                hexes,
                List.of(),
